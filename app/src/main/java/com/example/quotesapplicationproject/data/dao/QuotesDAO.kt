...
@Dao
interface QuotesDAO {
    @Query("SELECT * FROM quotes WHERE quote = :quote AND author = :author AND categoryId = :categoryId AND likedQuote=:isLikedQuote AND ratingId = :ratingId LIMIT 1")
    fun getQuoteByDetails(quote: String, author: String, categoryId: Int, ratingId: Int,isLikedQuote: Boolean): Quotes?
